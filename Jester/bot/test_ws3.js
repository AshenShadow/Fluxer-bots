...
async function run() {
    await connect('With Bot Prefix', {
        token: `Bot ${token}`,
        properties: { os: 'windows', browser: 'fluxerjs', device: 'fluxerjs' }
    });

    await connect('Different Properties', {
        token: token,
        properties: { $os: 'windows', $browser: 'fluxerjs', $device: 'fluxerjs' } // Discord format
    });

    fs.writeFileSync('test_ws_results2.json', JSON.stringify(results, null, 2));
}
