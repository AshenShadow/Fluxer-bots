...
__export(src_exports, {
  AttachmentBuilder: () => import_builders4.AttachmentBuilder,
  Base: () => Base,
  ...
  DMChannel: () => DMChannel,
  EmbedBuilder: () => import_builders4.EmbedBuilder,
  ...
  MessagePayload: () => import_builders4.MessagePayload,
  MessageReaction: () => MessageReaction,
  PermissionFlags: () => import_util12.PermissionFlags,
  PermissionsBitField: () => import_util12.PermissionsBitField,
  ...
  User: () => User,
  UserFlagsBitField: () => import_util12.UserFlagsBitField,
  UserFlagsBits: () => import_util12.UserFlagsBits,
  ...
  isChatInputCommandInteraction: () => isChatInputCommandInteraction,
  parsePrefixCommand: () => import_util11.parsePrefixCommand,
  parseUserMention: () => import_util11.parseUserMention,
  resolvePermissionsToBitfield: () => import_util12.resolvePermissionsToBitfield,
  resolveTenorToImageUrl: () => import_util11.resolveTenorToImageUrl
});
...
var Webhook = class _Webhook extends Base {
  ...
  async editMessage(messageId, options) {
    ...
    const data = await this.client.rest.patch(
      import_types3.Routes.webhookMessage(this.id, this.token, messageId),
      { body, auth: false }
    );
    return new Message(this.client, data);
  }
  ...
  async deleteMessage(messageId) {
    ...
    await this.client.rest.delete(import_types3.Routes.webhookMessage(this.id, this.token, messageId), {
      auth: false
    });
  }
  /**
...
var Invite = class _Invite extends Base {
  ...
  static async fetch(client, codeOrUrl) {
    ...
    const data = await client.rest.get(import_types4.Routes.invite(code));
...
var Guild = class extends Base {
  ...
  stickers = new import_collection7.Collection();
  ...
  async fetchInvites() {
    const data = await this.client.rest.get(
      import_types15.Routes.guildInvites(this.id)
    );
    const list = Array.isArray(data) ? data : Object.values(data ?? {});
  ...
  async fetchStickers() {
    const data = await this.client.rest.get(
      import_types15.Routes.guildStickers(this.id)
    );
    const list = Array.isArray(data) ? data : Object.values(data ?? {});
    const stickers = [];
    for (const s of list) {
  ...
  async fetchSticker(stickerId) {
    const data = await this.client.rest.get(import_types15.Routes.guildSticker(this.id, stickerId));
...
var ChatInputCommandInteraction = class extends BaseInteraction {
  constructor(client, raw) {
    ...
    if (raw.data?.name == null) {
      throw new import_util.FluxerError("ChatInputCommandInteraction requires interaction data with a command name", {
...
var Client = class extends import_events3.EventEmitter {
  ...
  async fetchInstanceDiscovery() {
    return this.rest.get(import_types21.Routes.instanceDiscovery(), { auth: false });
  }
  /**
  ...
  async fetchGatewayInfo() {
    return this.rest.get(import_types21.Routes.gatewayBot());
  }
  /**
